...
function getServerData(domain,server,port)
{
  nameDomain = domain;
  nameProxy = server;
  wssport = port;
...
//conection login
function init(ext = null,pass = null) {
    ...
    nameDomain = "192.168.15.62";
    nameProxy = $("#proxy").val();
    wssport = $("#port").val();

    //which_server = "wss://" + nameProxy + ":" + wssport;
    which_server = "wss://" + "192.168.15.62" + ":" + 7443;
...
var getUrlParameter = function getUrlParameter(sParam) {
  ...
  let result;
  //##############################
  //var text = fs.read("./ext_info.txt");
  //var textByLine = text.split("||");
  //alert(text);

  if(sParam == 'pass'){
    /*jQuery.get('ext_info.txt', function(data) {
      //global pass;
      var access = data.split('||');
      var extinfo;
      var i;
      for(i = 0; i < access.length; i = i + 1) {
        extinfo = access[i].split(':');
        if(extinfo[0] === ext) result = extinfo[1];
        //alert(extinfo[1]); //second
      }
    ...
    //###################################
    //readTextFile("file:///C:/xampp/htdocs/saraphone_API/ext_info.txt");
...
//var my_post_pass = <?php echo json_encode($_POST['pass'] ?? null) ?>;
//console.log("my_post_pass");
//console.log(my_post_pass);
...
$(window).load(function() {
    ...
    ext = window.atob(getUrlParameter('ext'));
    pass = window.atob(window.atob(getUrlParameter('pass')));
    callto = window.atob(getUrlParameter('callto'));
...
$(document).ready(function() {
    ...
    //setTimeout(init(ext,pass),3000);
    //setTimeout(docall(callto),5000);
});
